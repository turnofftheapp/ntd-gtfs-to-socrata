...
class Schedule(object):
  ...
  def GetDateRangeWithOrigins(self):
    ...
    #Seems like what izip does is return an array of tuples, but the replacement for izip does not. 
    minvalue, minindex = min(zip(starts, itertools.count())) # This line used to be: minvalue, minindex = min(itertools.izip(starts, itertools.count()))
    maxvalue, maxindex = max(zip(ends, itertools.count()))   # This line used to be: maxvalue, maxindex = max(itertools.izip(ends, itertools.count()))
