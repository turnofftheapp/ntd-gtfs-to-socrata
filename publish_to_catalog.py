...
def setMetadata(set,string):
  ...
  if string == "replace":
    dataset_name = set['agency_name'] + string

  description = "Agency name: " + set['agency_name'] + string + "\n"
...
#parameter4 is one row in the dataset that represents a "source" of data from some city somewhere
def createNewRevision(revision_source_type,parse_source,action_type,set):
  ##########################
  ### Step 1: Create new revision
  ##########################
  metadata = setMetadata(set,"s")
  permission = 'private'
...
def updateRevision(set,string):
  ...
  revision_url = f'{domain_url}/api/publishing/v1/revision'
  action_type = 'update'
  metadata = setMetadata(set,string)
  body = json.dumps({
    'metadata': metadata,
      'action': {
        'type': action_type
